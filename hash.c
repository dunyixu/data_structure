...
/*
 * $Id: hash.c,v 1.1.8.3 2009/03/19 04:33:21 yuyan Exp $
 *
...
{
    hash_table *hid = malloc(sizeof(hash_table));
    if (!hash_sz)
    ...
    memset(hid->buckets, 0, hid->size*sizeof(hash_link *));
    hid->cmp = cmp_func;
    hid->count = 0;
    hid->hash = hash_func;
    hid->next = NULL;
...
hashFreeItems(hash_table * hid, HASHFREE * free_func)
{
    ...
    int i = 0;
    int j;
    list = malloc(hid->count * sizeof(hash_link *));
    hash_first(hid);
